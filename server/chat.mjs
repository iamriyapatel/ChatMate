  ...
  return async function handle(request) {
    ...
    try {
      const response = await fetchImpl('https://openrouter.ai/api/v1/chat/completions', {
        ...
        body: JSON.stringify({ model, messages: [{ role: 'system', content: `You are ChatMate, a friendly, patient AI thinking partner. Be warm, clear and practical without excessive praise. Ask a brief clarifying question when needed. Be honest about uncertainty and never claim actions or access you do not have. Use readable paragraphs and lists; put code in fenced blocks. ${styles[style]}` }, ...contents.map(({ role, parts }) => ({ role: role === 'model' ? 'assistant' : role, content: parts.map(p => p.text).join('') }))], max_tokens: 4096 }),
